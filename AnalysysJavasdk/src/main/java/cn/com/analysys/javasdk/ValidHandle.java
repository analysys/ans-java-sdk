...
public class ValidHandle {
	...
	public static void checkParam(String eventName, Map<String, Object> properties) throws AnalysysException {
		...
		for (Map.Entry<String, Object> property : properties.entrySet()) {
			...
			if (property.getValue() instanceof String && property.getValue().toString().length() > idLength) {
				throw new AnalysysException(String.format("The property String value %s is too long, max length is %s.", property.getValue(), idLength));
			}
			//数组集合约束 数组或集合内最多包含100条,若为字符串数组或集合,每条最大长度255个字符
			if (property.getValue() instanceof List<?>) {
				List<?> valueList = (List<?>)property.getValue();
				if(valueList.size() > valueListLen){
					throw new AnalysysException(String.format("The property %s value %s, max number should be %s.", property.getKey(), property.getValue(), valueListLen));
				}
				for (Object vals : valueList) {
					if (!(vals instanceof String)) {
						throw new AnalysysException(String.format("The property %s should be a list of String.", property.getKey()));
					}
					if (((String) vals).length() > idLength) {
						throw new AnalysysException(String.format("The property %s some value is too long, max length is %s.", property.getKey(), idLength));
					}
				}
			}
	...
	public static void checkProperty(String distinctId, String eventName, Map<String, Object> properties, int commProLen) throws AnalysysException {
		...
		if (distinctId.toString().length() > idLength) {
			throw new AnalysysException(String.format("aliasId %s is too long, max length is %s.", properties.get(originalId), idLength));
		}
		...
		if(!eventName.startsWith(profileEventName) && !eventName.startsWith(aliasEventName)){
			if(properties.entrySet().size() + commProLen + connonParamLen > totalParamLen){
				throw new AnalysysException(String.format("Too many attributes. max number is %s.", (totalParamLen - commProLen - connonParamLen)));
			}
		} else {
			if(properties.entrySet().size() + connonParamLen > totalParamLen){
				throw new AnalysysException(String.format("Too many attributes. max number is %s.", (totalParamLen - connonParamLen)));
			}
		}
